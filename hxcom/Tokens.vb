Public Class Tokens
    ...
    Public Function TokenizeToCharacter(input As String) As List(Of TokenziedCharacter)
        Dim t As New List(Of TokenziedCharacter)
        Dim nli, cli As Integer
        For Each l As String In input
            Dim tc As New TokenziedCharacter
            If l = vbLf Then
            ...
            t.Add(tc)
        Next
        t.Add(New TokenziedCharacter("[EOF]", nli, cli, True))
        Return t
    End Function
...
Public Class TokenziedCharacter
...
Public Class Identifier
    ...
    Shared toks As New Tokens


    Public Shared Function IsKeyword(input As Word) As Boolean
        toks.LoadKeywordsKeyValuePair()

        If toks.KeywordsValue.Contains(input.GetWord) Then
            Return True
        Else
            Return False
        End If
...
Public Class IdentifiedWord
    ...
    Public Sub Add(tword As String, tlength As Integer, tline As Integer, tcol As Integer, itype As String)
        Word = tword : Line = tline : Column = tcol : Length = tlength : IdentifierType = itype
    End Sub
...
Public Class IdentifierTokens
    ...
    Public Function GetTokenKey(itoken As String, toks As Tokens) As String
    ...
    Public Function BuildTokens(input As List(Of TokenziedCharacter)) As List(Of IdentifiedToken)
        ...
        Dim ichar As String
        Dim w As String = ""
        Dim ws As New List(Of IdentifiedToken)
        ...
        For i = 0 To input.Count - 1
            ...
            If IsCharacterBreakpoint(ichar) And ecbrepakpoint = -1 Then
                ...
                If input(i).IsSpecialBreakpoint = True Then
                    ws.Add(New IdentifiedToken(ichar, 1, line, col - len, "[SBP]", True))
                Else
                    ws.Add(New IdentifiedToken(ichar, 1, line, col - len, "[SOMEBREAKPOINT]", False))
                End If
                ...
                Else
                    If ecbrepakpoint = 0 Then
                        ...
                        If IsEscapeCharacter(ecstring) = True Then
                            ...
                            ws.Add(New IdentifiedToken(ecstring, len, line, col - len, "", False))
                            ecstring = ""
                        Else
                            ecbrepakpoint = -1
                            len += 1
        ...
        Dim k As New List(Of IdentifiedToken)
        For i = 0 To ws.Count - 1
            Dim kd As New IdentifiedToken
        ...
        Dim p As New List(Of IdentifiedToken)
        Dim sindex As Integer = TokenDictionary.Count

        For i = 0 To k.Count - 1
            Dim pd As New IdentifiedToken
        ...
        Dim c As New List(Of IdentifiedToken)
        Dim qString As String = ""
        For i = 0 To p.Count - 1
            Dim cd As New IdentifiedToken
